...
#include "localization_pfilter.h"
...
*/
class LocalizationPFilter::Impl {
...
private:
    ...
    GameTime M_particles_generate_time;
...
public:
    ...
    */
    ...
    void generateParticles( const VisualSensor::MarkerT & marker,
                            ...
                            const double & self_face_err,
                            const GameTime & current );
    void updateParticles( const Vector2D & last_move,
                          const GameTime & current );
    void filterParticlesByMarkers( const VisualSensor::MarkerCont & markers,
                                   const double & self_face,
                                   const double & self_face_err,
                                   const GameTime & current );
    void filterParticlesByBehindMarkers( const VisualSensor::MarkerCont & behind_markers,
                                         const Vector2D & self_pos,
                                         const double & self_face,
                                         const double & self_face_err,
                                         const GameTime & current );
    void filterParticles( const VisualSensor::MarkerT & marker,
                          const MarkerID id,
                          const double & self_face,
                          const double & self_face_err,
                          const GameTime & current );
    ...
    void averageParticles( Vector2D * ave_pos,
                           Vector2D * ave_err );
    ...
    /*!
      ...
      \param obj_type used only to detect 'goal' or 'flag'
      \param pos target position
      \return marker ID

      This method is used to identify the behind marker object.
    */
    MarkerID getNearestMarker( const VisualSensor::ObjectType obj_type,
 ...
 */
MarkerID
LocalizationPFilter::Impl::getNearestMarker( const VisualSensor::ObjectType obj_type,
                                             const Vector2D & pos ) const
{
    // check closest behind goal
    if ( obj_type == VisualSensor::Obj_Goal_Behind )
    {
 ...
 */
double
LocalizationPFilter::Impl::getFaceDirByLines( const VisualSensor::LineCont & lines )
{
    if ( lines.empty() )
    {
 ...
 */
void
LocalizationPFilter::Impl::updatePointsByMarkers( const VisualSensor::MarkerCont & markers,
                                                  const double & self_face,
                                                  const double & self_face_err )
{
 ...
 */
void
LocalizationPFilter::Impl::updatePointsByBehindMarker( const VisualSensor::MarkerCont & markers,
                                                       const VisualSensor::MarkerCont & behind_markers,
                                                       const Vector2D & self_pos,
                                                       const double & self_face,
                                                       const double & self_face_err,
                                                       const GameTime & current )
{
    ...
    if ( points().empty() )
    {
        std::cerr << __FILE__ << ": " << current
                  << " (updatePointsByBehindMarker) re-generate points"
                  << std::endl;
        ...
        if ( points().empty() )
        {
            std::cerr << __FILE__ << ": (updatePointsByBehindMarker) no points by behind marker!!" << std::endl;
        }
 ...
 */
void
LocalizationPFilter::Impl::updatePointsBy( const VisualSensor::MarkerT & marker,
                                           const MarkerID id,
                                           const double & self_face,
                                           const double & self_face_err )
{
 ...
 */
void
LocalizationPFilter::Impl::averagePoints( Vector2D * ave_pos,
                                          Vector2D * ave_err )
{
 ...
 */
void
LocalizationPFilter::Impl::generatePoints( const VisualSensor::MarkerT & marker,
                                           const MarkerID id,
                                           const double & self_face,
                                           const double & self_face_err )
 ...
 */
void
LocalizationPFilter::Impl::resamplePoints( const VisualSensor::MarkerT & marker,
                                           const MarkerID id,
                                           const double & self_face,
                                           const double & self_face_err )
{
 ...
 */
void
LocalizationPFilter::Impl::getDirRange( const double & seen_dir,
                                        const double & self_face,
                                        const double & self_face_err,
                                        double * average,
                                        double * err )
 ...
 */
double
LocalizationPFilter::Impl::getFaceDirByMarkers( const VisualSensor::MarkerCont & markers )
{
...
*/
void
LocalizationPFilter::Impl::getDistRange( const double & seen_dist,
                                         const double & qstep,
                                         double * average,
                                         double * range )
{
 ...
 */
void
LocalizationPFilter::Impl::generateParticles( const VisualSensor::MarkerT & marker,
                                              const MarkerID id,
                                              const double & self_face,
                                              const double & self_face_err,
                                              const GameTime & current )
{
    ...
    if ( ! objectTable().getStaticObjInfo( marker.dist_,
                                           &ave_dist, &dist_error ) )
    {
        std::cerr << __FILE__ << " (generateParticles) marker dist error"
                  << std::endl;
        return;
    }
    ...
    const double min_dist = ave_dist - dist_error;
    const double dist_range = dist_error * 2.0;
    double dist_inc = std::max( 0.03, dist_error / 12.0 );
    const int dist_loop = std::max( 2,
                                    static_cast< int >( std::ceil( dist_range / dist_inc ) ) );
    dist_inc = dist_range / ( dist_loop - 1 );

    const double dir_range = dir_error * 2.0;
    const double circum = 2.0 * ave_dist * M_PI * ( dir_range / 360.0 );
    double circum_inc = std::max( 0.03, circum / 25.0 );
    const int dir_loop = std::max( 2,
                                   static_cast< int >( std::ceil( circum / circum_inc ) ) );
    const double dir_inc = dir_range / ( dir_loop - 1 );
    ...
    dlog.addLine( Logger::WORLD, v1, v2, "#ff00ff" );
    dlog.addLine( Logger::WORLD, v2, v4, "#ff00ff" );
    dlog.addLine( Logger::WORLD, v4, v3, "#ff00ff" );
    dlog.addLine( Logger::WORLD, v3, v1, "#ff00ff" );
#endif

    M_particles_generate_time = current;
}
 ...
 */
void
LocalizationPFilter::Impl::filterParticlesByMarkers( const VisualSensor::MarkerCont & markers,
                                                     const double & self_face,
                                                     const double & self_face_err,
                                                     const GameTime & current )
{
    ...
    if ( M_particles_generate_time == current )
    {
        ++marker;
    }
    ...
    for ( ;
          ...
          ++marker )
    {
        filterParticles( *marker, marker->id_,
                         self_face, self_face_err,
                         current );
    }
}
 ...
 */
void
LocalizationPFilter::Impl::filterParticlesByBehindMarkers( const VisualSensor::MarkerCont & behind_markers,
                                                           const Vector2D & self_pos,
                                                           const double & self_face,
                                                           const double & self_face_err,
                                                           const GameTime & current )
{
    ...
    // matching behind marker
    MarkerID marker_id = getNearestMarker( behind_markers.front().object_type_,
                                           self_pos );
    ...
    dlog.addText( Logger::WORLD,
                  __FILE__" (filterParticlesByBehindMarker)"
                  " update by BEHIND marker" );
#endif

    filterParticles( behind_markers.front(),
                     marker_id,
                     self_face, self_face_err,
                     current );
}
 ...
 */
void
LocalizationPFilter::Impl::filterParticles( const VisualSensor::MarkerT & marker,
                                            const MarkerID id,
                                            const double & self_face,
                                            const double & self_face_err,
                                            const GameTime & current )
{
    ...
    if ( ! objectTable().getStaticObjInfo( marker.dist_,
                                           ...
                                           &dist_error ) )
    {
        std::cerr << __FILE__ << " (filterParticles) unexpected marker distance "
                  << marker.dist_ << std::endl;
        return;
    }
    ...
    dlog.addLine( Logger::WORLD, v1, v2, "#aaaaaa" );
    dlog.addLine( Logger::WORLD, v2, v4, "#aaaaaa" );
    dlog.addLine( Logger::WORLD, v4, v3, "#aaaaaa" );
    dlog.addLine( Logger::WORLD, v3, v1, "#aaaaaa" );
#endif
    ...
    {
        generateParticles( marker, id, self_face, self_face_err, current );
    }
}
 ...
 */
void
LocalizationPFilter::Impl::averageParticles( Vector2D * ave_pos,
                                             Vector2D * ave_err )
{
    ave_pos->assign( 0.0, 0.0 );
    ave_err->assign( 0.0, 0.0 );
    ...
    *ave_pos /= static_cast< double >( M_particles.size() );
 ...
 */
LocalizationPFilter::LocalizationPFilter()
    : M_impl( new Impl() )
{
 ...
 */
// void
// LocalizationPFilter::updateParticles( const Vector2D & self_vel,
//                                       const GameTime & current )
// {
//     M_impl->updateParticles( self_vel, current );
// }
 ...
 */
bool
LocalizationPFilter::updateBySenseBody( const BodySensor & )
{
    return true;
}
 ...
 */
bool
LocalizationPFilter::localizeSelf( const VisualSensor & see,
                                   const double & self_face,
                                   const double & self_face_err,
                                   Vector2D * self_pos,
...
#if 0
bool
LocalizationPFilter::localizeSelf2( const VisualSensor & see,
                                    const double & self_face,
                                    const double & self_face_err,
                                    Vector2D * self_pos,
                                    Vector2D * self_pos_err,
                                    const GameTime & current )
{
    ...
    M_impl->filterParticlesByMarkers( see.markers(),
                                      self_face,
                                      self_face_err,
                                      current );
    M_impl->averageParticles( self_pos, self_pos_err );

    if ( ! see.behindMarkers().empty() )
    {
        M_impl->filterParticlesByBehindMarkers( see.behindMarkers(),
                                                *self_pos,
                                                self_face,
                                                self_face_err,
                                                current );
    ...
    //M_impl->resampleParticles();

    return self_pos->valid();
}
#endif
 ...
 */
bool
LocalizationPFilter::localizeBallRelative( const VisualSensor & see,
                                           const double & self_face,
                                           const double & self_face_err,
                                           Vector2D * rpos,
                                           Vector2D * rpos_err,
 ...
 */
bool
LocalizationPFilter::localizePlayer( const VisualSensor::PlayerT & from,
                                     const double & self_face,
                                     const double & self_face_err,
                                     const Vector2D & self_pos,
