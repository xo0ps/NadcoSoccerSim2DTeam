...
*/
bool
Bhv_OffmidDefmidMove::execute( rcsc::PlayerAgent * agent )
{

	/*
	if( agent->world().ball().pos().x > 20.0 )
	{
		std::vector< rcsc::Vector2D >pos;
		const rcsc::PlayerPtrCont & team = agent->world().teammatesFromSelf();
		...
		for( rcsc::PlayerPtrCont::const_iterator  tm = team.begin(); tm != team_end; tm++ )
		{
			if( (! (*tm)) || (*tm)->posCount() > 8 )
				continue;
			pos.push_back( (*tm)->pos() );
		}
		
		for( uint i = 0 ; i < pos.size() - 1 ; i++ )
			for( uint j = i ; j < pos.size() ; j++ )
				if( pos[i].x > pos[j].x )
				{
					rcsc::Vector2D tmp = pos[i];
					pos[i] = pos[j];
					pos[j] = tmp;
				}
		...
		for( uint i = 0 ; i < pos.size() ; i++ )
			if( pos[i].x < agent->world().self().pos().x )
				count++;
		
		if( count <= 6 )
		{
		//	if( doAttackMove( agent ) )
		//		return true;
		}
	}
	*/
    ...
    */
	...
	std::cout<<"offmid move false"<<std::endl;
	return false;
}
...
void
Bhv_OffmidDefmidMove::goToReceivablePoint( rcsc::PlayerAgent * agent )
{
