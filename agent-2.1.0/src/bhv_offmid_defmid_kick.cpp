...
*/
bool
Bhv_OffmidDefmidKick::execute( rcsc::PlayerAgent * agent )
{
	if( Strategy::i().rc() )
	{
		return RC( agent );
	...
	/*
	if( wm.self().pos().x > 30.0 && wm.self().pos().absY() < 15 )
	{
		const PlayerObject * goalie = wm.getOpponentGoalie();
		if( ( ! goalie ) || goalie->isGhost() )
		{
	...
	*/
		...
		/*----------------------------------------------------*/
			...
			if( wm.self().pos().x > wm.offsideLineX() - 27 )
			{
				...
				if( counter == 4 )
				{
					...
					std::cout<<"offmid through"<<std::endl;	
					counter = 0;
					agent->setNeckAction( new rcsc::Neck_TurnToLowConfTeammate() );
					return true;
				}
				...
				if( Body_ThroughPass::test1( agent , &target , &first_speed , &unum ) )
				{
					...
					agent->setNeckAction( new rcsc::Neck_TurnToLowConfTeammate() );
					std::cout<<"offmid "<<counter<<std::endl;
					return true;
				}
				else
					counter = 0;
		...
		
		    {
				...
				if ( Body_KickToCenter::get_best_point( agent, NULL ) && wm.ball().pos().x > 26.0 )
			    {
			...
			std::cout<<"offmid false"<<std::endl;
			agent->setNeckAction( new rcsc::Neck_TurnToLowConfTeammate() );
		    return false;
	...
	if ( Strategy::i().off_planner() )
	{
	    if(wm.self().pos().x > 0)
	    {
			...
			{
				std::cout<<"offdef kick isShootLine"<<std::endl;
			    agent->setNeckAction( new rcsc::Neck_TurnToLowConfTeammate() );
			    return true;
			}
			...
			{	
				std::cout<<"offdef kick keepShootChance"<<std::endl;
			    agent->setNeckAction( new rcsc::Neck_TurnToLowConfTeammate() );
			    return true;
			}
	    }
    ...
    {
        const rcsc::Vector2D body_dir_drib_target( 50 , wm.self().pos().y );
	...
	rcsc::Vector2D drib_target( 40 , -12 );
	if( wm.self().pos().y > 0 )
		drib_target = rcsc::Vector2D( 40 , 12 );
...
void
Bhv_OffmidDefmidKick::offensivePlan( rcsc::PlayerAgent * agent )
{
	if ( Strategy::i().old_pass() )
    {	
		if( Body_LeadingPass("old").execute( agent ) )
    ...
    {
		if( agent->world().ball().pos().x >= 0 && agent->world().ball().pos().absY() > 10  )
		{
...
*/
bool
Bhv_OffmidDefmidKick::RC( rcsc::PlayerAgent * agent )
{
	...
	if( Body_LeadingPass2().execute( agent ) )
	{
		return true;
	}
	...
	*/
	...
	std::cout<<"offdefmid false"<<std::endl;
	agent->setNeckAction( new rcsc::Neck_TurnToLowConfTeammate() );
    return false;
}
